...
half MipGaussianBlendWeight(int level)
{
    half sigma = 2.0;
    half sigma2 = sigma * sigma;
    half c = 2.0 * 3.141592654 * sigma2;
    half numerator = (1 << (level << 2)) * log(4.0);
    half denorminator = c * ((1 << (level << 1)) + c);
